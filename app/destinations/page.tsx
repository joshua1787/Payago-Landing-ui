        ...
        <main className="min-h-screen bg-[#04060A]">
                ...
                <div className="max-w-6xl mx-auto px-6">
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {destinations.map((dest) => (
                            <Link key={dest.slug} href={`/destinations/${dest.slug}`} className="group block bg-white/[0.02] border border-white/5 rounded-2xl overflow-hidden hover:border-white/10 hover:-translate-y-1 transition-all duration-300">
                                ...
                                <div className="p-5">
                                    <div className="flex items-start justify-between mb-2">
                                        <div>
                                            <h2 className="text-xl font-bold text-white group-hover:text-[#C9A962] transition-colors">{dest.name}</h2>
                                    ...
                                    <div className="flex flex-wrap gap-1.5 mb-4">
                                        {dest.tags.map(t => (
                                            <span key={t} className="text-xs px-2 py-0.5 rounded-full bg-white/5 text-white/40 border border-white/5">{t}</span>
                                        ))}
                                    </div>
                                    <div className="flex items-center justify-between pt-3 border-t border-white/5">
                                        <div>
                                            <div className="text-white font-semibold text-sm">{dest.budget}</div>
            ...
            <section className="py-16 border-t border-white/5">
                <div className="max-w-4xl mx-auto px-6 text-center">
                    <h2 className="text-3xl font-bold text-white mb-4">Don't see your destination?</h2>
                    <p className="text-white/50 mb-8">PayaGo AI can plan group trips to any destination in the world — just describe it and the AI builds the full itinerary in 30 seconds.</p>
