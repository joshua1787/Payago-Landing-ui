import { Sparkles, Users, CreditCard, Mic, Check, Zap, Globe, Map } from "lucide-react"
...
// Static preview keeps the section light while scrolling.
...
function VotingPreview() {
    return (
        <div className="space-y-2.5">
            {votingPeople.map((p, i) => (
                <div key={i} className="flex items-center gap-3 py-2 px-3 rounded-2xl border bg-slate-50 border-slate-200">
                    ...
                    <span className="text-sm flex-1 text-slate-600">
                        {p.name}
                    </span>
                    <div className="w-5 h-5 rounded-full flex items-center justify-center bg-emerald-50 border border-emerald-200">
...
// Static rings for voice card. Continuous pulse animations compete with scroll.
function VoiceRings() {
    return (
        <div className="relative flex items-center justify-center h-28">
            {[0, 1, 2].map((i) => (
                <div
                    key={i}
                    className="absolute rounded-full border border-[#C9A962]/20"
                    style={{
                        width: `${(i + 1) * 56}px`,
                        height: `${(i + 1) * 56}px`,
                    }}
            ...
            <div className="relative z-10 w-14 h-14 rounded-2xl bg-gradient-to-br from-[#C9A962] to-[#E5C77D] flex items-center justify-center shadow-sm">
...
export function BentoSection() {
    return (
        <section className="scroll-stable-section relative py-28 overflow-hidden">
            <div className="absolute inset-0 bg-[#F4F6FB]" />
            <div className="absolute top-1/3 left-1/4 w-[600px] h-[400px] bg-[radial-gradient(ellipse,rgba(124,92,255,0.10),transparent_70%)] pointer-events-none" />
            ...
            <div className="relative z-10 max-w-6xl mx-auto px-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    {/* ── Large: AI itinerary hero card ── */}
                    <div className="md:col-span-2 relative border-2 border-cyan-300 rounded-3xl overflow-hidden group transition-colors duration-200 hover:border-cyan-400 min-h-[320px] flex flex-col" style={{ background: "linear-gradient(145deg, #ddf6ff 0%, #edfbff 35%, white 100%)" }}>
                        {/* Dominant header */}
                        <div className="px-7 pt-7 pb-5" style={{ background: "linear-gradient(135deg, #00D4FF28 0%, #00D4FF0A 100%)" }}>
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-cyan-500 shadow-md shadow-cyan-500/30">
                                    ...
                                    <span className="text-white text-[10px] font-bold uppercase tracking-wider">Draft</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="px-7 pb-7 flex-1 flex flex-col">
                            ...
                            <div className="mt-6 flex items-end justify-between">
                                <div>
                                    <div className="text-5xl font-black text-slate-900 tracking-tight leading-none">AI</div>
                                    <div className="text-cyan-600 text-sm mt-1.5 font-semibold">from one sentence to a review-ready 7-day draft</div>
                                </div>
                    ...
                    {/* ── Tall: Group voting ── */}
                    <div className="md:row-span-2 relative border-2 border-violet-200 rounded-3xl overflow-hidden group transition-colors duration-200 hover:border-violet-300 flex flex-col" style={{ background: "linear-gradient(145deg, #ede9ff 0%, #f5f3ff 40%, white 100%)" }}>
                        {/* Colored header */}
                        ...
                        <div className="px-7 pb-7 mt-4">
                            <div className="p-4 bg-violet-500 rounded-2xl shadow-sm">
                                <div className="text-white text-sm font-semibold mb-2">Shared progress</div>
                                <div className="w-full bg-violet-400/40 rounded-full h-2 overflow-hidden">
                                    <div className="h-full rounded-full bg-white transition-[width] duration-1000" style={{ width: "80%" }} />
                                </div>
                    ...
                    {/* ── Bottom row left: Early access ── */}
                    <div className="relative border-2 border-amber-200 rounded-3xl overflow-hidden group transition-colors duration-200 hover:border-amber-300" style={{ background: "linear-gradient(145deg, #fef9ec 0%, #fffbf0 40%, white 100%)" }}>
                        ...
                        <div className="px-7 pt-7 pb-5" style={{ background: "linear-gradient(135deg, #C9A96222 0%, #C9A96208 100%)" }}>
                            ...
                            <div className="text-xl font-bold text-slate-900 mb-1">Early access</div>
                            <div className="text-amber-700 text-sm font-medium">Join the waitlist. No card required.</div>
                        </div>

                        <div className="px-7 pb-7">
                            <div className="space-y-2 mb-4">
                                {["AI trip planning", "Group coordination", "Split-cost tracking", "Smart assist"].map((feat) => (
                    ...
                    <div className="relative border-2 border-rose-200 rounded-3xl overflow-hidden group transition-colors duration-200 hover:border-rose-300" style={{ background: "linear-gradient(145deg, #fff1f2 0%, #fff5f5 40%, white 100%)" }}>
                        {/* Colored header strip */}
                ...
                {/* Bottom strip — feature signals */}
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
                    {[
                        { value: "Guides", label: "Destination inspiration", icon: Globe, color: "#00D4FF", bg: "#e0f9ff", border: "#bae6fd" },
                        { value: "Groups", label: "Shared votes and decisions", icon: Users, color: "#7C5CFF", bg: "#ede9ff", border: "#c4b5fd" },
                        { value: "Map", label: "Timeline and map itinerary views", icon: Map, color: "#4AD7A2", bg: "#dcfce7", border: "#bbf7d0" },
                    ].map((s) => (
                        <div key={s.label} className="flex items-center gap-4 rounded-2xl px-5 py-4 border-2 transition-colors duration-200 hover:bg-white" style={{ background: s.bg, borderColor: s.border }}>
