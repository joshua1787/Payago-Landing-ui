            ...
            <div className="space-y-3 p-5">
                ...
                {["Day 1: Tokyo Arrival", "Day 2: Asakusa & Shibuya", "Day 3: Hakone day trip"].map((d, i) => (
                    <div key={i} className="flex items-center gap-3 animate-fade-up" style={{ animationDelay: `${i * 0.2}s` }}>
                        ...
                        <span className="text-slate-600 text-xs font-medium">{d}</span>
                    </div>
                ))}
                <div className="mt-4 pt-3 border-t border-slate-100">
                    <div className="flex items-center gap-2 text-[10px] text-slate-300 font-mono">
                        <div className="w-16 h-1 bg-gradient-to-r from-cyan-400/40 to-transparent rounded-full" />
                ...
                {[
                    ...
                    <div key={i} className={`flex items-center gap-3 rounded-xl p-3 text-xs transition-all ${item.dragging ? 'bg-violet-50 border border-violet-200 scale-[1.02] translate-x-1 shadow-md shadow-violet-100' : 'bg-slate-50 border border-slate-100'}`}>
                        ...
                        <div className="flex-1">
                            <div className="text-slate-700 font-medium text-[12px]">{item.name}</div>
                            <div className="text-slate-400 text-[10px] mt-0.5">{item.time}</div>
                        </div>
                        {item.dragging && <GripVertical className="w-3.5 h-3.5 text-violet-400" />}
                    </div>
                ))}
            </div>
        ),
            ...
            <div className="p-5 space-y-2.5">
                {[
                    ...
                    <div key={i} className="flex items-center gap-3 bg-slate-50 border border-slate-100 rounded-xl p-3.5 text-xs">
                        <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-white border border-slate-100">
                            <b.Icon className="w-4 h-4" style={{ color: b.iconColor }} />
                        </div>
                        <div className="flex-1">
                            <div className="text-slate-700 font-semibold text-[12px]">{b.name}</div>
                            <div className="text-slate-400 text-[10px] mt-0.5">{b.type}</div>
                        </div>
                        <span className="text-emerald-500 font-bold text-[12px]">{b.price}</span>
                    </div>
                ))}
